...
<ParamField path="STRIX_SAFETY_MODEL" type="string">
  ...
  when unset. Routing review to a small, cheap model keeps the agents doing the
  testing on the main model.
</ParamField>

<ParamField path="SAFETY_LLM_API_KEY" type="string">
  Optional provider key for the safety model.
</ParamField>

<ParamField path="SAFETY_LLM_API_BASE" type="string">
  Optional custom API base URL for the safety model. Use when the safety model
  runs on a different endpoint than the main model.
</ParamField>

<ParamField path="SAFETY_LLM_EXTRA_HEADERS" type="string">
  Optional JSON object of extra HTTP headers sent on every safety-model request,
  e.g. `{"X-Feature-Key":"value"}`. A dedicated safety model never inherits
  `LLM_EXTRA_HEADERS`; set this when its endpoint needs custom headers.
</ParamField>
