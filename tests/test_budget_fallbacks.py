...
@pytest.mark.asyncio
async def test_fallback_carries_subagent_credential_per_call() -> None:
    # A cross-provider fallback must re-resolve credentials so it doesn't run on
    # the exhausted model's key (env vars are global per provider).
    llm = LlmSettings(
        reasoning_effort="none",
        api_key="root-key",
        subagent_api_key="fallback-key",
        model_budgets_usd={"openai/gpt": 1},
        model_fallbacks={"openai/gpt": "anthropic/claude"},
    )
    hooks = ReportUsageHooks(model="openai/gpt", llm_settings=llm)
    coordinator = AgentCoordinator()
    agent = SimpleNamespace(name="strix", model="openai/gpt", model_settings=ModelSettings())
    await coordinator.register("root", "strix", None, model="openai/gpt")
    ...
    with (
        patch("strix.core.hooks.get_global_report_state", return_value=state),
        pytest.raises(ModelFallbackError),
    ):
        await hooks.on_llm_end(context, agent, response)

    assert agent.model == "anthropic/claude"
    assert agent.model_settings.extra_args["api_key"] == "fallback-key"
